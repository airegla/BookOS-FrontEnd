...
- `ConsignaPage.jsx` — flujo de consignacion: liquidaciones, conciliador de sabanas y devoluciones. En el alta de devoluciones, cada renglon elige **«Sale de»** (Auto: consigna→firme / Consigna: baja actual + original / Firme). Interconectado con el Secretario (contexto + consulta + observaciones LLM de cada documento).
...
- `RemitosPage.jsx` — ingreso de remitos en esquema "cabecera + tabla de items" + cruce de faltantes. Cabecera con **Tipo de ingreso (FIFE)**: Consigna (actual + original) o Firme; avisa que **el remito nunca genera deuda** (solo mueve libros y se valoriza). Acciones por fila: Ver, Cruzar faltantes, **Confirmar ingreso** y **Anular** (revierte el stock). Interconectado con el Secretario: inyecta el borrador y escucha instrucciones (refrescar, cruzar) para que el agente opere la vista.
