...
- `PropuestasVentaPage.jsx` — propuestas de lectura del CRM (doc 06 P5/D6): el operario elige cliente, cantidad y una aclaracion, el asistente arma la propuesta (tematicas + afinidad + stock, seleccion y motivos por LLM) y queda en BORRADOR para revisarla antes de mandarla por mail. Con el boton **Medir resultado** se corre el outcome del backend (doc 06 P7) y la columna **Resultado** muestra cuanto de lo ofrecido compro cada cliente ("compro 2/5 · $..."), con "sin medir" cuando la propuesta ya se envio pero todavia no se midio.
